...
//constants and global variables
  //LCD
    ...
    const int n_r=110, n_g=125, n_b=125;
    const int o_r=0, o_g=255, o_b=0;
    const int c_r=255, c_g=123, c_b=0;
    const int w_r=255, w_g=0, w_b=0;
    enum farba {neutral=0, ok=1, caution=2, warning=3};
    farba current_color=caution;
    LiquidCrystal lcd(rs, en, d4, d5, d6, d7);
  //MQ7
    const int MQ7_output = A1, MQ7_control=9;
    int CO_value=0;
    float MQ7_change=0;
    enum MQ7_status {heating, cooling};
    MQ7_status actual_status=heating;
    const int border_ok=255, border_caution=270;
...
void MQ7_stuff()
{
  if(millis()>MQ7_change)
  {
    ...
    {
      actual_status=cooling;
      analogWrite(MQ7_control, 100);
      MQ7_change=millis()+9000;
    }
    else if(actual_status==cooling)
    {
      actual_status=heating;
      analogWrite(MQ7_control, 255);
  ...
  if(CO_value<border_ok)
    current_color=max(current_color, ok);
  else if(CO_value>border_caution)
    current_color=max(current_color, warning);
  else
    current_color=max(current_color, caution);
}
...
float compute_BPM()
{
  ...
  if((k*(sum_Rs/max_bpms)+q)>=border_ok_spo2)
    current_color = ok;
  else if((k*(sum_Rs/max_bpms)+q)<=border_caution_spo2)
